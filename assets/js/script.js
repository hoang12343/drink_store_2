  ...
  const formSwitch = {
    ...
    init() {
      if (!this.registerPage || !this.loginPage) {
        console.warn(
          "Register or Login page not found in DOM. Form switching disabled."
        );
        return;
      }
      ...
      links.forEach((link) => {
        link.addEventListener("click", (e) => {
          e.preventDefault();
          const targetPage = link.getAttribute("href").includes("register")
            ? "registerPage"
            : "loginPage";
          this.switchTo(targetPage);
        });
      });
      const currentPage = window.location.search.includes("page=register")
        ? "registerPage"
        : "loginPage";
    ...
    initRegisterForm() {
      ...
      const validations = {
        ...
        username: {
          ...
          message: "Tên đăng nhập phải từ 4-20 ký tự",
        },
        email: {
          test: (value) => this.validateEmail(value),
          errorId: "emailError",
          message: "Vui lòng nhập email hợp lệ",
        },
        phone: {
          test: (value) => this.validatePhone(value),
          errorId: "phoneError",
      ...
      form.addEventListener("submit", (e) => {
        ...
        Object.entries(validations).forEach(
          ([fieldName, { test, errorId, message }]) => {
